...
class Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        # encoder_1: [batch, 16, 32, in_channels] => [batch, 8, 16, ngf]
        self.layers.add_module(name='conv2d0', module=torch.nn.Conv2d(kernel_size=4, stride=2, in_channels=128, out_channels=ngf, padding=1))
...
class FencingModel(nn.Module):
    ...
    def __init__(self, players_in_same_channel, use_optical_flow, use_pose_img):
        ...
        self.rnn_player = ConvLSTM(input_size=(16, 32),
                                     input_dim=128,#256
                                     hidden_dim=[64],
                                     kernel_size=(3, 3),
                                     num_layers=1,
        ...
        # self.rnn_right_player = ConvLSTM(input_size=(16, 32),
        #                                 input_dim=128,#256
        #                                 hidden_dim=[64],
        #                                 kernel_size=(3, 3),
        #                                 num_layers=1,
        ...
        self.rnn_overall = ConvLSTM(input_size=(16, 32),
                                         input_dim=64*2,
                                         hidden_dim=[128],
                                         kernel_size=(3, 3),
                                         num_layers=1,
    ...
    def forward(self, frames_pose_tensor, frames_optical_flow_tensor=None):
        ...
        outputs_player0, state_player0 = self.rnn_player(encodings_player0)
        outputs_player1, state_player1 = self.rnn_player(encodings_player1)
        ...
        #TODO: maybe add decoder here
        last_output = outputs_overall[0][:, -1]#.view(batch_size, -1)
        res = self.decoder(last_output)
        res = res.view(batch_size, -1)
        res = self.relu(self.fc1(self.dropout(res)))
